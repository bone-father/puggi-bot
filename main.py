...
async def on_message(message):
    ...
    if "puggi" in message.content.lower() or '695356435168493598' in message.content:
        await message.channel.send(file=discord.File('images/puggi/(' + str(random.randint(1, 227)) + ').jpg'))
...
async def anthony(ctx):
    await ctx.send(file=discord.File('images/anthony/(' + str(random.randint(1, 7)) + ').jpg'))
...
@bot.command()
async def pug(ctx):
    await ctx.send(file=discord.File('images/pug/(' + str(random.randint(1, 41)) + ').jpg'))
